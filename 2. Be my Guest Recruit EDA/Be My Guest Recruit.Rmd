---
title: "Kaggle 2. Be My Guest Recruit"
...
This is an extentive EDA for the [Recruit Restaurant Visitor Forecasting](https://www.kaggle.com/c/recruit-restaurant-visitor-forecasting) competition with the powers of [tidy R](http://tidyverse.org/) and [ggplot2](http://ggplot2.tidyverse.org/).
...
We load a range of libraries for general data wranling and general visualization together with more specialized tools.
```{r message=FALSE}
...
## Helper function
We use *multiplot* function, courtesy of [R Cookbooks](http://www.cookbook-r.com/Graphs/Multiple_graphs_on_one_page_(ggplot2)/) to create multi-panel plots. We also make use of a brief helper function to compute binomial confidence intervals.
...
```{r echo=FALSE}
# fuction to extract binomial confidence levels
get_binCI <- function(x,n) as.list(setNames(binom.test(x,n)$conf.int, c("lwr", "upr")))
```

## Load data
We use *data.table's* fread function to spped up reading in the data:
```{r echo=FALSE}
on_kaggle <- 1
...
if (on_kaggle == 0){
  ...
  rpath <- "C:/Users/kojikm.mizumura/Desktop/Data Science/Kaggle/data/"
  wpath <- "C:/Users/kojikm.mizumura/Desktop/Data Science/Kaggle/rrv-weather-data/"
  wdpath <- "C:/Users/kojikm.mizumura/Desktop/Data Science/Kaggle/rrv-weather-data/1-1-16_5-31-17_Weather/1-1-16_5-31-17_Weather/"
}
```

```{r warning=FALSE, results=FALSE}

air_visits <- as.tibble(read_csv("data/air_visit_data.csv"))
air_reserve <- as.tibble(read_csv("data/air_reserve.csv"))
hpg_reserve <- as.tibble(read_csv("data/hpg_reserve.csv"))
air_store <- as.tibble(read_csv("data/air_store_info.csv"))
hpg_store <- as.tibble(read_csv("data/hpg_store_info.csv"))
holidays <- as.tibble(read_csv("data/date_info.csv"))
store_ids <- as.tibble(read_csv("data/store_id_relation.csv"))
test <- as.tibble(read_csv("data/sample_submission.csv"))
...
We find this file contains the *visitors* numbers for each *visit\_date* and *air\_store\_id*. The date feature should be transformed into a time-series format. There are 829 different stores, which is a small data set:
```{r}
air_visits %>% 
...
we find that *air\_store* info includes the name of the particular cuisine along with the name of the area.
...
Here we have a first look at the distributions of the feature in our individual data files before combining them for a more detailed analysis. This initial visualizations will be the foundation on which we build our analysis.

## Air visits

We start with the number of visits to the *air* restaurants. Here we plot the total number of visitors per day over the full *training* time range together with the median visitors per day of the week and months of the year:
```{r split=TRUE, fig.align="default", warning=FALSE, fig.cap="Fig.1", out.width="100%"}
...
air_visits %>% 
  ...
  ggplot(aes(visit_date, all_visitors))+
  geom_line()+
  geom_smooth(method="loess",col="blue",span=1/7)+
  labs(y="All visitors",x="Date")
```
...
## Air reservations
Let's see how our reservations data compares to the actual visitor numbers. We start with the *air* restaurants and visualize their visitor through reservations for each day, alongside the hours of these visitors and the time between making a reservation and visiting the restaunrant:
```{r split=FALSE, fig.align="default", warning=FALSE, fig.cap="Fig.3",out.width="100%"}

foo <- air_reserve %>% 
  mutate(reserve_date =date(reserve_datetime),
         reserve_hour=hour(reserve_datetime),
         reserve_wday=wday(reserve_datetime,label=T),
         visit_date=date(visit_datetime),
         visit_hour=hour(visit_datetime),
         visit_wday=wday(visit_datetime,label=T),
...
foo <- hpg_reserve %>% 
  mutate(
    reserve_date = data(reserve_datetime),
    reserve_hour = hour(reserve_datetime),
    visit_date   = date(visit_datetime),
    visit_hour   = hour(visit_datetime),
    diff_hour = time_length(visit_datetime-reserve_datetime, unit="hour"),
...
- Here the visits after reservation follow a more ordely pattern, with clear spike in Dec 2016. As above for the *air* data, we also see reservations visits dropping ofv as we get closer to the end of the time frame.
- Again, most reservations are for dinner, and we see another nice 24-hour pattern for making these servations. It's worth noting that here the last few hours before the visit don't see more volume than the 24 or 48 hours before. This is in stark contrast to the *air* data.

## Air store

After visualizing the temoporal aspect, let's now look at the spatial information. Note that according to the [data description](https://www.kaggle.com/c/recruit-restaurant-visitor-forecasting/data) the latitude and longitude are the area to which the store belongs. This is meant to discourage us from identifying specific restaurants. I would be surprised if nobody tried anyway though.
...
In the same way as for the *air* stores above, we also create an interactive map for the different *hpg* resutaurants:
...
Here we have truncated the *hpg\_area_name* to 20 characters to make the plot more readable.
...
Our first plot of the multi-feature space deals with the average number of *air* restaurant *visitors* broken down by type of cuisin; i.e., the *air\_genre\_name*. We use a facet plot to distinguish the time series for the 14 categories. Note the logarithmic y-axis:
...
Here each color corresponds to a day of the week.Red-ish colors are the weekend, while the cooler color are the middle of the week. Monday is dark orange. 
...
foo <- air_visits %>%
  mutate(calendar_date = as.Date(visit_date)) %>%
...
air_store %>% 
  ...
  theme(legend.position = "buttom", axis.text.x=element_text(angle=45, hjust=1, vjust=0.9))
...
Now we will break down the discrepancy between `visitors - reserve_visitors` over time, look at the overall histograms and visualize the *air\_reserve* vs *hpg\_reserve* numbers separately. Here, the time series for *air* (blue) and *hpg* (red) are offset vertically by 150 and 250 (see the solid black baselines):
...
The next step is to derive new features from the existing ones and the purpose of these new features is to provide additional predictive power for our goal to forecast *visitors* numbers. Those new features can be as simple as deriving the *day of the week* or the *month* from a *date* column; something that we have already used in Fig. 1 of our visual exploration. Or they can take a more complex shape from the interplay of several related variables. This section collects and studies these new features.
...
- The differences in mean `log(visitor)` numbers between the days of the wekk are much smaller than the correspondong uncertainties. This is confirmed by the density plots that show a strong overlap. Still, there is a trend toward higher visitor numbers on the weekend that might be useful.
...
We find: 
- There is a strong impact of the weekend vs weekdays for *genres* such as "KAraoke" or "International Cuisine", while others show very little change over the days of the week (e.g., "Japanese Food").
...
We find theres is no significant difference, althou we see a slight trend for more *visitors* compared to *reservations* on the weekend, especially, Sunday.
...
Here, we plot the histogram of the distances from the medians for all the restaurants in the *air* and *hpg* data. These distributions will suggest a grouping into 5 different bins with ranges of increasing distance. We then compute the mean `log1p` visitor numbers for the 5 groups and compare them:

```{r split=FALSE, fig.align = 'default', warning = FALSE, fig.cap ="Fig. 26", out.width="100%", eval=FALSE}
...
The extreme values in this plot can now be studied in details, and we will choose some of them as examples in our next section.
...
A popular method for forecasting is the *autoregressive integrated moving average* model; short [ARIMA](https://en.wikipedia.org/wiki/Autoregressive_integrated_moving_average) model. This kinf of model consits of three building block which parameterised by the three indices *p, d, q* as ARIMA(p, d, q):
...
arima.fit <- auto.arima(
  tsclean(ts(visits_train$visitors, frequency = 7)),
...
max_date <- max(air_visits$visit_date)
splot_date <- max_date - pred_len
all_visits <- tibble(
  visit_date = seq(min(air_visits$visit_date), max(air_visits$visit_date), 1))
...
Prohpet offers a decomposition plot, where we can inspect the additive components of the model: trend, yearly seasonality, and weekly cycles:
